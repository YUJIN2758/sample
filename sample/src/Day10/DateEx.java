...
// Date 객체는 시스템에서 현재 날짜를 가져온 정보를 가지고 있다 1/1000
		Date now = new Date();
		...
		String day1 ="2025/03/10";
		String day2 = "2025/09/16";
		...
		//Date.getTime() : 1970년 1월 1일 0시0분0초 Date에 지정된 날짜/시간까지의 ms(밀리초) 누적값
		...
		System.out.println("종강 남은 시간(/시)  :" +gapHours);
		System.out.println("종강 남은 시간(/분)  :" +gapMin);
		System.out.println("종강 남은 시간(/초) :" +gapSec);
		
		Date today = new Date();
		System.out.println("오늘 : " + today);
		//5일 후 
		long dayTime =(1000*60*60*24); //1일 /ms
		long DayStimeValue =5 * dayTime; //5일
		Date afterDays = new Date();
		afterDays.setTime(today.getTime() + DayStimeValue);
